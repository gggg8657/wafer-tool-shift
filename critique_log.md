...
<details>
<summary><strong>108 entries — click to expand</strong></summary>
...
- **100.** [What should anyone actually use? Neither headline result answers it](#100-what-should-anyone-actually-use-neither-headline-result-answers-it)
- **101.** [Measuring the caveat I argued past](#101-measuring-the-caveat-i-argued-past)
- **102.** [Making the hypothesis record auditable, since it is the only thing that distinguishes prediction from observation](#102-making-the-hypothesis-record-auditable-since-it-is-the-only-thing-that-distinguishes-prediction-from-observation)
- **103.** [Three failed attempts to detect a verdict in prose, and the structure that replaced them](#103-three-failed-attempts-to-detect-a-verdict-in-prose-and-the-structure-that-replaced-them)
- **104.** [H76 confirmed on all four protocols, and the exception that refines entry 99](#104-h76-confirmed-on-all-four-protocols-and-the-exception-that-refines-entry-99)
- **105.** [H77 falsified: the interaction is on the seen-geometry half, not the unseen one](#105-h77-falsified-the-interaction-is-on-the-seen-geometry-half-not-the-unseen-one)
- **106.** [The resize-free encoder, and why a confirmation here would be weak](#106-the-resize-free-encoder-and-why-a-confirmation-here-would-be-weak)
- **107.** [Two sections numbered 7.1, and a guard whose first version was worse than the defect](#107-two-sections-numbered-71-and-a-guard-whose-first-version-was-worse-than-the-defect)
...
### 100. What should anyone actually use? Neither headline result answers it

Entry 99 showed the two surviving positive results do not compose. That leaves a
question neither of them addresses and a reader would ask first: of the four
normalisation-by-pooling combinations, which one?

All four are at eight seeds on `lot` and `size`, so it is answerable with no new
run. Testing each protocol's best against the other three:

**`lot`, macro-F1 — best is GroupNorm + mean-and-max, 0.8888.** It beats
GroupNorm + mean by +0.0150 (p = 0.01197) and BatchNorm + mean by +0.0312
(p = 0.00016), and does **not** beat BatchNorm + mean-and-max (+0.0055,
p = 0.21507).

**`size`, macro-F1 — best is GroupNorm + mean, 0.8462.** The unmodified
baseline. It beats BatchNorm + mean (p = 0.01632) and nothing else: GroupNorm +
mean-and-max is 0.0281 behind at p = 0.16177, BatchNorm + mean-and-max 0.0218
behind at p = 0.24522.

**On `size` the architectural change this project spent most of its length
establishing has a negative point estimate, and the plain baseline is the best
of the four.** That is not a new measurement — it is the same `size` numbers
reported since section 2.0 — but arranging the four combinations in one table
makes the practical consequence visible in a way the two separate comparisons
did not.

So the honest recommendation is narrower than either positive result reads like:
**if the wafers you will see share their geometry with your training set, add
the max; if they will not, nothing here is shown to beat the plain baseline.**
On `lot` the normalisation choice stops mattering once the max is present, which
is the same fact as entry 99 from the other direction.

A note on what this is and is not. Picking the best of four and testing it
against the rest is a selection procedure, and the p-values are not corrected
for it — the winner was chosen by the same data that scores it. The comparisons
that matter here survive that objection because they were pre-specified as
individual hypotheses (`meanmax` vs `mean` was H73 and its predecessors,
GroupNorm vs BatchNorm was an earlier arm), and the table is a rearrangement
rather than a new test. The one genuinely new statement — that the plain
baseline is best on `size` — is supported by *nothing being established over
it*, which is a claim about absence and does not need the selection correction.

`scripts/pooling_all_protocols_bn.sh` is running: 48 cells giving `cnn_bn`
pooling arms on `iid` and `lot_time`, so the design is complete on all four
protocols rather than half. **H76 on record:** the normalisation-by-pooling
interaction replicates on both, at seven or eight of eight seeds. The
interesting failure would be `iid` — the only protocol with no shift at all. If
the interaction vanishes there it is a fact about generalisation rather than
about the two normalisations, and entry 99's explanation is wrong.

### 101. Measuring the caveat I argued past

Entry 100 arranged the four normalisation-by-pooling combinations, tested each
protocol's best against the other three, and then noted the objection: the
winner was chosen by the same data that scores it, so those p-values are not
family-wise. I answered it with an argument — the comparisons that matter were
pre-specified as individual hypotheses — and moved on. That was the wrong move
twice over: an argument where a measurement was available, and my own argument
in my own favour.

The correction is a max-statistic permutation test, and the design matters. All
four arms ran the same eight seeds, so a seed fixes the split, the
initialisation and the batch order for all of them. Permuting all thirty-two
values would discard that pairing and inflate the noise floor, so each draw
permutes the four values *within* every seed, recomputes the six pairwise mean
differences, and records the largest. 4!^8 is about 1.1e11, so this is Monte
Carlo — 20,000 draws, fixed seed, standard errors reported, and labelled
inexact, which this project has been strict about elsewhere and should not drop
when the answer is inconvenient.

**On `lot` macro-F1, the pooling comparison does not survive.**

| best vs | difference | family-wise p | MC SE |
|---|---|---|---|
| BatchNorm + mean-and-max | +0.0055 | 0.8727 | 0.0024 |
| GroupNorm + mean | +0.0150 | **0.1369** | 0.0024 |
| BatchNorm + mean | +0.0312 | 0.0000 | 0.0000 |

`meanmax` over `mean` is +0.0150 at a per-comparison p of 0.01197 and a
family-wise p of **0.1369**.

**On `lot` `Scratch` F1 it survives comfortably: +0.0473 at family-wise
p = 0.0009.**

That split is the useful part, and it is not a rescue. This project has said
since section 2.0 that the pooling effect is concentrated in `Scratch`, that the
mechanism predicted a thin-class effect *before* the run, and that macro-F1
follows only where one class can move a nine-class average. The correction says
the same thing from the other side: the class-specific effect is robust to
having selected the winner, and the average is not.

Both numbers are answers to real and different questions. As a **planned single
comparison** — does adding a max help, at fixed normalisation, which is what
was specified in advance — the per-comparison p is correct and the macro-F1
effect stands. As part of **choosing the best of four after seeing all four**,
it does not. A reader picking a configuration is doing the second thing, so the
honest headline for that reader is the `Scratch` number.

And the reason to trust the `Scratch` number is not that it is larger. It is
that it was predicted. A post-hoc correction cannot distinguish an effect that
was specified in advance from one that was noticed; only the record can, and
that is the whole reason hypotheses go in the script headers here before the
runs.

The ratchet caught `0.01197` typed into this very section — the seventh time
this session, and again a number read off a terminal while writing prose about
it.

### 102. Making the hypothesis record auditable, since it is the only thing that distinguishes prediction from observation

Entry 101 concluded that a post-hoc correction cannot tell an effect specified in
advance from one that was noticed, and that only the record can. That record has
lived all weekend in two places — a prediction in each sweep's header before it
launches, an outcome in a critique entry afterwards — and in one summary written
by hand.

Entry 85 already named the risk in words: "a loop that only records the
predictions it gets right is not keeping a record, it is keeping a highlight
reel." Words are what drifts. **The failure mode is silent by construction:** a
hypothesis stated before a run that goes badly and is never mentioned again
breaks nothing, invalidates no number, and leaves behind a list of predictions
that came true.

`hypothesis_ledger.py` extracts every `H<n>` stated in a sweep header and every
one the critique log gives an outcome for, and reports the difference. Twenty-two
hypotheses are stated; every one whose sweep has finished has an outcome; H76 is
still running and is marked as such rather than as a gap.

**Three attempts, and the first two were too generous to me.**

*It counted itself.* The ledger's own docstring names H63 and H70 to explain
what it does, so those read as stated by `hypothesis_ledger.py`. Excluded.

*A mention is not an outcome.* Counting any `H<n>` in any entry made H76 — still
running — read as scored by entry 100, the entry that launched it. Requiring a
verdict word nearby did not fix it either, because **a prediction's own text
contains outcome words**: entry 100 says "H76 on record: the interaction
replicates". The discriminator is the statement marker. A mention followed by
"on record" or "before the run" is a statement whatever surrounds it, and only
the rest can be a scoring.

*Running is not unscored.* With that fixed the ledger correctly flagged H76, and
`--strict` would then have failed on a hypothesis legitimately in flight — a
check that fires during normal work is one you learn to pass with `--strict` off,
which is entry 79's lesson. Completion now comes from the stage's own log, so
the check fails on *finished and never scored* and stays quiet on *running*.

What the ledger cannot do is say whether an outcome was scored **correctly**. It
verifies that a prediction was given a verdict, not that the verdict was honest —
H74's disjunction, which I satisfied on a literal reading and reported as a
failure anyway, is exactly the case no automated check would catch. That
distinction is in the script's docstring, because a guard that seems to certify
more than it does is worse than none.

`check_all.py` is twelve checks; `guard_audit.py` fourteen of fifteen.

### 103. Three failed attempts to detect a verdict in prose, and the structure that replaced them

Entry 102 built a ledger checking that every stated hypothesis was scored. It
took three attempts and all three were too generous to me, in escalating and
instructive ways.

*Counting any mention.* H76 — stated in entry 100 and still running — read as
scored by the entry that launched it.

*Requiring a verdict word nearby.* Also failed, because **a prediction's own
text contains outcome words**: entry 100 says "H76 on record: the interaction
replicates". Fixed by treating a mention followed by "on record" or "before the
run" as a statement whatever surrounds it.

*Tightening the window.* Failed on entry 102 — the entry *about* the ledger.
An unrelated "does not" fell inside 400 characters of an H76 mention. Prose
discussing the record is indistinguishable from prose scoring it.

At that point I was on my fourth regex against natural language, which is the
signal to stop. **The problem is not the pattern, it is that the verdict was
never written down as a verdict.** So it is now:
`state/hypothesis_outcomes.json` holds one line per hypothesis with a verdict
from a declared vocabulary, the entry that argues it, and a note. The judgement
stays human — H74 was satisfied on a literal reading of a disjunction and
recorded as a failure anyway, and no pattern would ever have made that call —
and the machine's job is narrowed to refusing to let a stated hypothesis have no
recorded verdict. That it can do reliably.

**What the completed record says, which is the reason to keep one:**
...
**Nine of seventeen held as stated — 53%.** Four were partly right, generally
with the direction correct and a threshold wrong. Four failed outright.

A little over half is not flattering and that is the point. A record showing
near-perfect foresight would mean the predictions were being written to be safe
or written after the fact, and either way it could not do the job this one
exists for: distinguishing what was specified in advance from what was noticed.
Entry 101 needed exactly that distinction to decide which of two p-values to
believe, and could only appeal to the record because the record was kept
honestly — including the four failures and the disjunction I should not have
written.

Also added `hypothesis_sharpness.py`, which lints for the one defect H74 had: a
prediction that names a direction and then adds a clause covering the opposite.
It flags H74 and none of the other twenty-one, and the test asserts it does not
flag the four that read similarly and are fine — "if either does, the
withdrawal comes back out" is a falsification clause, and "not established
either way" is an adverb. A lint firing on those would cover most of the record
and be turned off, which is entry 79's failure mode.

### 104. H76 confirmed on all four protocols, and the exception that refines entry 99

`pooling_all_protocols_bn.sh` finished: 48 cells giving `cnn_bn` pooling arms on
`iid` and `lot_time`, completing the normalisation-by-pooling design.

**H76, on record before the run:** the interaction replicates on both, at seven
or eight of eight seeds each. It does.

| protocol | GN−BN under `mean` | under `meanmax` | change | seeds | p |
|---|---|---|---|---|---|
| `lot` | +0.0162 | +0.0055 | -0.0106 | 8/8 | 0.00781 |
| `size` | +0.0521 | -0.0063 | -0.0583 | 8/8 | 0.00781 |
| `iid` | +0.0184 | +0.0062 | -0.0121 | 7/8 | 0.01562 |
| `lot_time` | +0.0562 | +0.0233 | -0.0329 | 7/8 | 0.01562 |

**The stated failure mode did not occur, and it was the informative one.** I
wrote that if the interaction needed *some* shift to appear it should vanish on
`iid` — the only protocol with none — and that this would make it a fact about
generalisation rather than about the two normalisations, and would mean entry
99's explanation was wrong. On `iid` the gap shrinks on 7 of 8 seeds at
p = 0.01562. So it is a property of the normalisations and the pooled statistic,
which is what entry 99 claimed and could not then show.

**And four protocols refine what entry 99 concluded from two.** That entry said
the GroupNorm advantage is "established under `mean` on both protocols and
established under `meanmax` on neither". With all four: established under `mean`
on **4 of 4**, shrinks under `meanmax` on **4 of 4**, and survives it on **1** —
`lot_time`, at +0.0233, p = 0.0062.

`lot_time` starts from the largest gap of the four (+0.0562) and there is more
of it left after the shrink. So the honest form is not "the normalisation choice
stops mattering once you add the max" but **"most of what it was worth is
available from the pooling instead — and on the protocol with the largest
normalisation effect, not all of it."** Entry 99's version was true of the two
protocols it had and overstated as a general claim, which is the same
generalising-from-a-subset error as entry 97, arrived at from a different
direction: there I took one triple for the distribution, here two protocols for
four.

The `Scratch` interaction remains `size`-specific: not significant on `lot`
(p = 0.17969) or `iid` (p = 0.23438), significant on `size` (p = 0.02344) and on
`lot_time` (p = 0.02344). Both of those hold geometry out or test on a narrow
geometry slice, which is suggestive and is not something these runs establish.

H76's verdict is recorded in `state/hypothesis_outcomes.json`, taking the tally
to 10 confirmed, 4 partly, 4 falsified of 18 scored.

### 105. H77 falsified: the interaction is on the seen-geometry half, not the unseen one

Entry 104 left one thread labelled suggestive-and-not-established: the `Scratch`
norm-by-pooling interaction fires on `size` (p = 0.02344) and `lot_time`
(p = 0.02344) and not on `lot` (0.17969) or `iid` (0.23438), and the two where it
fires are the two that hold geometry out or test on a narrow geometry slice.

Four protocols differ in too many ways at once for that to be evidence. But
`run_bench.py` scores every `lot_time` test set twice — on the wafers whose
geometry appeared in training and on those whose did not — and records per-class
F1 for both halves. So exposure can be varied with the models, the training data
and the seeds held fixed, which is a far tighter test than anything across
protocols, and it needed no new runs.

**H77, stated before computing:** within `lot_time`, the interaction is larger
on the unseen-geometry half.

| `lot_time` test half | `cnn_gn` | `cnn_bn` | interaction | seeds | p |
|---|---|---|---|---|---|
| seen geometry | +0.0346 | +0.0779 | **+0.0433** | 8/8 | **0.00781** |
| unseen geometry | +0.0539 | +0.0411 | -0.0128 | 3/8 | 0.70312 |

**It is the other way round.** The interaction is established on the *seen* half
at the floor of the test, and null on the unseen half. H77 is falsified, and the
geometry-exposure explanation for the across-protocol pattern is dropped.

**And I am not installing the reverse.** The two halves are not shown to differ
from each other: -0.0561, 5 of 8 seeds, p = 0.17188. The unseen half is 7,630 of
43,237 test wafers with a visibly wider per-seed spread (|interaction| ranging
0.0138–0.1612 against 0.0035–0.1089 on the seen half), so its null is partly a
power statement rather than an effect. What the test establishes is that my
prediction was wrong — enough to drop the explanation, not enough to put its
opposite in its place.

That asymmetry is the same one entry 98 used and is worth restating because it
keeps coming up: a result is adequate evidence *against* a claim you were making
and weak evidence *for* a new one. Dropping "geometry exposure drives the
interaction" needs only that its prediction failed. Adopting "seen geometry
drives it" would need the halves to differ, and they do not.

So the `Scratch` interaction's 2-of-4 pattern is now unexplained rather than
explained-by-geometry, which is a worse-sounding and more accurate position. The
`macro_f1` interaction, by contrast, is on all four protocols and needs no
explanation of that kind.

H77 recorded as falsified: 10 confirmed, 4 partly, 5 falsified of 19 scored.

### 106. The resize-free encoder, and why a confirmation here would be weak

H65 established with no model involved that `resize_nearest` couples a defect's
apparent width to its native geometry. Every attempt to act on that has failed:
dilating the first conv block is catastrophic whatever the dilation (H66, H69),
and the `size` behaviour it was supposed to explain did not replicate across
encoders (H74). The measurement stands and its consequences keep not
materialising.

There is one encoder in this repository that avoids the problem rather than
patching it. `spectral` is a Fourier neural operator: learned weights multiply a
fixed number of low-frequency coefficients, so the same weights apply to a 25x27
and a 53x58 wafer with **no resampling at all**.

The comparison has to be a degradation, not a level. `spectral` is worse than
the CNN on `Scratch` everywhere in absolute terms — around 0.55 on `lot`
against the CNN's 0.73 to 0.78 — and that is capacity and inductive bias, not
resampling. What the mechanism predicts is a smaller *drop* when geometry is
held out.

The CNN reference is already measured and is itself the mechanism's signature:

| arm | `lot` | `size` | drop | seeds worse |
|---|---|---|---|---|
| CNN + mean-and-max | 0.7764 | 0.6660 | **-0.1104** | 7/8 |
| CNN + mean | 0.7292 | 0.7087 | -0.0205 | 6/8 |

The arm with the max statistic loses five times as much as the arm without it.
So the question for `spectral` is sharp: does a resize-free encoder behave like
`mean` (small drop) or like `meanmax` (large drop)?

**H78, before the run:** `spectral` degrades less from `lot` to `size` than the
`meanmax` CNN does.

**And the asymmetry in what this can show is worth stating before the data
arrives, not after.** `spectral` differs from the CNN in the operator, the
capacity and the inductive bias, not only in the resampling. So a confirmation
is weak evidence for the mechanism — many differences could produce it — while
a falsification is strong evidence against, because the mechanism's prediction
would have failed on the one architecture built to avoid the thing it blames.
There is exactly one resize-free encoder here, and one is not a controlled
comparison.

If it does fail, the honest position gets weaker than the paper's current one:
the model-free measurement would remain true about apparent width and have **no
demonstrated consequence for any trained model at all** — two failed remedies
and a failed prediction. That is where the evidence has been pointing since H66,
and this is the last cheap test that could turn it around.

16 cells running: eight seeds on `lot` and `size` under a fresh `spec8` tag
rather than topping up the three-seed `sess2` arms, since those were measured in
a different session and this project has been bitten once by mixing.

**Postscript to 106: two false positives from my own guards, both worth keeping.**

`coverage_check.py` flagged `resize_free_degradation.json` as an orphan, because
I had written the scoring script before wiring its output into the paper. The
right response was not to silence it but to pre-wire the paper for both
outcomes, which is the practice H73 and H74 established and which I had skipped
here. Both branches are now in `paper.py` with the data absent, so whichever way
H78 lands the document reports it without my choosing afterwards.

Then the typed-decimal ratchet fired on **`7.955`** — a section number, not a
measurement. A real false-positive class: three-decimal section numbers look
like four-significant-figure results. But the useful part is what it exposed. I
had been numbering inserted sections 7.9, 7.95, 7.955 precisely to avoid
renumbering, because renumbering trips `section_diff.py`. Five subsections had
accumulated in an order where the decimals no longer matched the document —
7.9, 7.95, **7.94**, 7.96, 7.97.

So I had been letting a guard's alarm shape the artefact rather than the
reverse, and the result was numbering nobody could read. Renumbered to 7.1
through 7.5 in document order, `section_diff` fired as designed, and `--accept`
recorded it with a diff in git. That is the workflow working: the guard's job is
to make a structural change visible and require a decision, not to make
structural changes expensive enough to avoid.

### 107. Two sections numbered 7.1, and a guard whose first version was worse than the defect

Reading the paper's structure rather than its content, after entries 97 through
106 each inserted a section: **`paper_draft.md` had two sections numbered 7.1**
— the pre-existing "The target, answered" and "How much each of our nulls could
have shown", which my renumbering last turn moved on top of it. All twelve
checks passed. `section_diff.py` tracks headings by *text*, so a number
collision is invisible to it.

Two smaller things came with it. Heading levels had drifted — `4.1` and `4.2`
were `###` beside `4.3` as `##` — and section 5's only children were `5.5` and
`5.6`, numbered to avoid disturbing anything rather than to describe anything.
Both are artefacts of inserting sections where insertion was cheapest.

Fixed by putting the five methodological subsections in their own top-level
section, which is where they belonged anyway: they measure the instrument and
the operators, not the methods, and had been sitting under "The multi-label
question" for no reason but proximity. `WEEKEND.md` needed the same treatment —
its `## 2.0` preceded its own parent `## 2`, and a `2.3a` I had inserted came
*before* `2.3`. Renumbered with the prose cross-references moved in step.

**And the guard I wrote to catch this was, in its first three versions, worse
than the defect.** Its regex required whitespace immediately after the digits,
so it matched none of the `## 2. Title` headings and then reported every
subsection as parentless — fourteen problems in a document whose numbering was
correct. Two further attempts at the ordering check put the letter suffix in the
same tuple as the integers, so `(2, 0) <= (1, "")` compared an int against a
str and the whole thing crashed.

Had I trusted the first version I would have "fixed" fourteen non-problems and
missed the one real duplicate, which is a worse outcome than never writing it.
The test now pins all four behaviours: a duplicate is caught, a `## 2. Title`
parent is *found*, a lettered sibling is neither a duplicate nor out of order,
and a wrong heading level is flagged.

**The test's own first version also failed, and for its own reason** — it wrote
the fixture inside a `with tempfile.NamedTemporaryFile(...)` and read it before
the handle closed, so the guard saw an empty file and reported nothing. The
guard was right and the test was wrong, which is now the third time this session
that has happened (entries 88, 90, and here). The pattern is that I write the
assertion against what I expect the code to do rather than against observable
behaviour, and the fixture is where that leaks.

`check_all.py` is thirteen checks; `guard_audit.py` sixteen of seventeen.
